...
if font_path is None:
    ...
    # We'll use English labels instead
...
codes = [code.strip() for code in codes if isinstance(code, str) and code.strip()]
codes = sorted(list(set(codes)))
print("\nUnique codes:", codes)

# Get unique stages
stages = df['stage_en'].unique()
...
for stage_en, stage_cn in stage_map.items():
    stage_data = df[df['stage'] == stage_cn]
        ...
        if isinstance(row['code'], str) and row['code'].strip():
            for code in row['code'].split(','):
                if code.strip() in codes:
                    stage_code_counts.loc[stage_map[stage_cn], code.strip()] += 1
...
for region_en, region_cn in region_map.items():
    region_data = df[df['Region'] == region_cn]
    # For rows with code, split and count each code
    for _, row in region_data.iterrows():
        if isinstance(row['code'], str) and row['code'].strip():
            for code in row['code'].split(','):
                if code.strip() in codes:
                    region_code_counts.loc[region_map[region_cn], code.strip()] += 1
...
top_docs = df['document-id'].value_counts().head(10).index.tolist()

# Create shorter document labels for better visualization
doc_labels = {}
for i, doc_id in enumerate(top_docs):
    doc_labels[doc_id] = f"Doc {i+1}: {doc_id[:10]}..."

doc_code_counts = pd.DataFrame(
    0,
    index=[doc_labels[doc] for doc in top_docs],
    columns=codes
)
...
for doc_id in top_docs:
    ...
    for _, row in doc_data.iterrows():
        if isinstance(row['code'], str) and row['code'].strip():
            for code in row['code'].split(','):
                if code.strip() in codes:
                    doc_code_counts.loc[doc_labels[doc_id], code.strip()] += 1
...
ax = sns.heatmap(stage_code_counts, annot=True, cmap=cmap, fmt="d")
...
plt.figure(figsize=(12, 6))
ax = sns.heatmap(region_code_counts, annot=True, cmap=cmap, fmt="d")
...
ax = sns.heatmap(doc_code_counts, annot=True, cmap=cmap, fmt="d")
